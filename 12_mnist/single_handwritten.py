...
minwid = int(sys.argv[1])

model = load_model('models/mnistdense.h5')
...
def find_number(contours, thresh):
	num_str = '[ ]'
	if len(contours) > 0:
...
if(video.isOpened()):
	while True:
		...
		img2 = img.copy()
		img2 = imutils.resize(img2, width=min(minwid, img2.shape[1]))
		img_gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
		img_gray = imutils.resize(img_gray, width=min(minwid, img_gray.shape[1]))
		...
		num_str = find_number(contours, thresh)
